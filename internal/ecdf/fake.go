...
func (c *fakeChunkStore) WriteChunk(serviceId int, indicatorId int, timestamp time.Time, chunk []byte) error {
	...
	entry := &fakeEntry{timestamp: timestamp, chunk: chunk, baseline: true}
...
func (c *fakeChunkStore) CountEligibleChunks(_ context.Context, serviceID, indicatorID int) (int, error) {
	entries := c.chunks[serviceID][indicatorID]
	count := 0
